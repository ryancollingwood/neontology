---
title: Data types
description: "Primitive and nested data types supported in Ladybug for properties and expressions"
---

import { LinkCard, CardGrid } from '@astrojs/starlight/components';

Ladybug supports a set of primitive and nested data types both for node and relationship properties as
...
ISO/IEC 9834-8:2005, and related standards. Ladybug follows [PostgreSQL's implementation](https://www.postgresql.org/docs/13/datatype-uuid.html)
for the `UUID` format.
...
└───────────┘
```
Ladybug's CLI returns an empty cell to indicate nulls.
...
```cypher
// Midnight in UTC-10
lbug> RETURN timestamp("1970-01-01 00:00:00-10") as x;
```
```table
┌─────────────────────┐
│ x                   │
...
lbug> RETURN timestamp("1970-01-01 00:00:00.004666") as x;
```
```table
┌────────────────────────────┐
│ x                          │
...
Ladybug follows [DuckDB's implementation](https://duckdb.org/docs/sql/data_types/interval) for the
interval format.
...
:::note[Note]
Starting in v0.15.0, the JSON data type is natively available in Ladybug. Only the table functions to manipulate JSON are in the [JSON extension](/extensions/json).
:::

Using the native `JSON` type or the `json` extension, you can model and operate on properties as JSON natively via the `JSON`
logical type, rather than as a string. We recommend storing JSON data using the `JSON` type instead of `STRING`.
