...
is not used in the examples below, so we discuss how Ladybug infers the variable names and data types of
...
Some of these configurations are also by default [automatically detected](/import/csv#csv-configurations) by Ladybug when scanning CSV files.
...
Then if you run the following query, Ladybug will infer the column names `name` and `age` from the first line of the CSV:
...
Since Parquet files contain schema information in their metadata, Ladybug will always use the available
...
Ladybug allows zero-copy access to Pandas DataFrames. The variable names and data types of scanned columns
...
:::note[Note]
Pandas can use either a NumPy or Arrow backend - Ladybug can natively scan from either backend.
:::

### Polars

Ladybug can also scan Polars DataFrames via the underlying PyArrow layer. The rules for determining the
...
Ladybug can scan directly scan JSON files `LOAD FROM`, but it requires installing the JSON extension.
...
in Ladybug's JSON extension to read the JSON string and convert its contents into the types needed
for the Ladybug table.
...
Alternatively. you can handle the JSON string in your client language and pass it to Ladybug.
...
Once you have the JSON structure, you can handle query the properties as structs using the dot notation in Ladybug.
...
By default, Ladybug will infer the column names and data types from the scan source automatically.
...
- Ladybug will throw an exception if the given number of columns in `WITH HEADERS` does not the match number of columns in the file.
- Ladybug will always try to cast to the type specified header. An exception will be thrown if the
